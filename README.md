...
PMHub 是一套模块化的产品经理求职 Skills：从目标岗位 JD 出发，把岗位要求翻译成可验证证据，再落到简历和面试。
...
可以跳过不需要的阶段。只想分析岗位时直接调用 JD 组件；已有定稿简历且马上面试时直接调用面试组件。
...
套件吸收了两个已有公开项目，并为 PMHub 做了命名空间、Codex 元数据、职责边界和阶段衔接适配。固定来源提交见 [SOURCES.md](SOURCES.md)。原始项目仍可独立使用。
